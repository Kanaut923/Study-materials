...
# 第二章：数据模型 (Data Models)

这一章决定了你能不能画对E-R图，能不能设计出合理的表。
...
## 2. 概念模型：E-R图（Entity-Relationship）

**这是设计题的核心，必须会画！**

### 2.1 三要素
1.  **实体 (Entity)**：**矩形**。客观存在的东西（人、物、概念）。例如：`学生`。
2.  **属性 (Attribute)**：**椭圆**。实体的特征。例如：`学号`、`姓名`。
3.  **联系 (Relationship)**：**菱形**。实体之间的交互。例如：`选课`。
...
## 3. 逻辑结构设计 (E-R图转关系模式) —— *计算/设计题核心*

做题时，就是要把你画的E-R图变成具体的表。有一套死规则：
...
基础的 `SELECT *` 不会考大题，大题通常涉及**多表、分组、子查询**。
...
# 第四部分：核心语言篇——SQL 怎么写（重中之重）
**(对应第9章：SQL数据查询)**

考试大题必考写SQL语句，请务必熟练掌握以下模版。
...
# 第9章：SQL 数据查询 (The Art of Querying)

## 核心心法：SQL的执行顺序
在写SQL之前，你必须先死记硬背这个**执行顺序**。很多人写错SQL，是因为以为电脑是按书写顺序执行的，其实不然。
